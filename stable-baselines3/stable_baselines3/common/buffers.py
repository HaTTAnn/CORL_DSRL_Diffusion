...
class ReplayBuffer(BaseBuffer):
    ...
    rewards: np.ndarray
    dones: np.ndarray
    chunk_execs: np.ndarray
    denoising_steps: np.ndarray
    timeouts: np.ndarray
    ...
    def __init__(
        ...
        self.chunk_execs = np.ones((self.buffer_size, self.n_envs), dtype=np.float32)
        self.denoising_steps = np.ones((self.buffer_size, self.n_envs), dtype=np.float32)
    ...
    def add(
        ...
        self.chunk_execs[self.pos] = np.array([
            max(1.0, float(info.get("chunk_exec", info.get("chunk_requested", 1.0))))
            for info in infos
        ], dtype=np.float32)
        self.denoising_steps[self.pos] = np.array([
            max(1.0, float(info.get("rollout_steps", info.get("denoising_steps", 1.0))))
            for info in infos
        ], dtype=np.float32)
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> ReplayBufferSamples:
        ...
        if self.use_noise_actions:
            data = (
                ...
                self.chunk_execs[batch_inds, env_indices].reshape(-1, 1),
                self.denoising_steps[batch_inds, env_indices].reshape(-1, 1),
