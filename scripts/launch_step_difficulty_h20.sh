...
case "$TASK" in
  can)
    ...
    TARGET_NFE="${TARGET_NFE:-1.70}"
    NFE_TARGET_LOWER="${NFE_TARGET_LOWER:-1.55}"
    NFE_TARGET_UPPER="${NFE_TARGET_UPPER:-1.90}"
    ;;
  square)
...
ACTOR_COMPUTE_LAMBDA_WARMUP="${ACTOR_COMPUTE_LAMBDA_WARMUP:-0.05}"
...
FAILED_EPISODE_COST_WEIGHT="${FAILED_EPISODE_COST_WEIGHT:-0.15}"
COST_GATE_MODE="${COST_GATE_MODE:-step_success}"
COST_START_STEP="${COST_START_STEP:-10000}"
COST_WARMUP_STEPS="${COST_WARMUP_STEPS:-90000}"
COST_SUCCESS_THRESH_1="${COST_SUCCESS_THRESH_1:-0.65}"
COST_SUCCESS_THRESH_2="${COST_SUCCESS_THRESH_2:-0.80}"
COST_OPEN_RATE="${COST_OPEN_RATE:-0.08}"
COST_CLOSE_RATE="${COST_CLOSE_RATE:-0.10}"
COST_NO_ROLLBACK="${COST_NO_ROLLBACK:-0}"

USER_RANGE_SUCCESS_THRESH_1="${RANGE_SUCCESS_THRESH_1:-}"
USER_RANGE_SUCCESS_THRESH_2="${RANGE_SUCCESS_THRESH_2:-}"
USER_RANGE_OPEN_RATE="${RANGE_OPEN_RATE:-}"
USER_RANGE_ACTUATOR_FLOOR="${RANGE_ACTUATOR_FLOOR:-}"
...
USER_DIFFICULTY_MARGIN_GATE_FLOOR="${DIFFICULTY_MARGIN_GATE_FLOOR:-}"
USER_DIFFICULTY_PRIOR_GATE_FLOOR="${DIFFICULTY_PRIOR_GATE_FLOOR:-}"
...
MAX_DENOISING_STEPS="${MAX_DENOISING_STEPS:-15}"
...
DIFFICULTY_MARGIN_GATE_FLOOR="${DIFFICULTY_MARGIN_GATE_FLOOR:-0.0}"
DIFFICULTY_HARD_STEPS_TARGET="${DIFFICULTY_HARD_STEPS_TARGET:-15}"
DIFFICULTY_EASY_STEPS_TARGET="${DIFFICULTY_EASY_STEPS_TARGET:-3}"
...
DIFFICULTY_QUANTILE_HINGE_WEIGHT="${DIFFICULTY_QUANTILE_HINGE_WEIGHT:-0.0}"
if [[ "$TASK" == "can" ]]; then
...
case "$VARIANT" in
  ...
  advz_strong|qstdz_strong)
    ...
    if [[ "$TASK" == "can" ]]; then
      ...
      DIFFICULTY_MODE_MARGIN_WEIGHT=0.70
      PREPRIOR_RESIDUAL_SCALE=0.35
      SCHEDULE_RESIDUAL_SCALE=0.20
      DIFFICULTY_PRIOR_SIGNAL_SCALE=1.25
      DIFFICULTY_SIGNAL_SCALE=1.00
      RANGE_SUCCESS_THRESH_1="${USER_RANGE_SUCCESS_THRESH_1:-0.35}"
      RANGE_SUCCESS_THRESH_2="${USER_RANGE_SUCCESS_THRESH_2:-0.55}"
      RANGE_OPEN_RATE="${USER_RANGE_OPEN_RATE:-0.15}"
      RANGE_ACTUATOR_FLOOR="${USER_RANGE_ACTUATOR_FLOOR:-0.50}"
      DIFFICULTY_SUCCESS_THRESH_1="${USER_DIFFICULTY_SUCCESS_THRESH_1:-0.35}"
      DIFFICULTY_SUCCESS_THRESH_2="${USER_DIFFICULTY_SUCCESS_THRESH_2:-0.55}"
      DIFFICULTY_SUCCESS_OPEN_RATE="${USER_DIFFICULTY_SUCCESS_OPEN_RATE:-0.25}"
      DIFFICULTY_MARGIN_GATE_FLOOR="${USER_DIFFICULTY_MARGIN_GATE_FLOOR:-0.10}"
      DIFFICULTY_PRIOR_GATE_FLOOR="${USER_DIFFICULTY_PRIOR_GATE_FLOOR:-0.20}"
    fi
    ;;
  *)
...
echo "loss: mode=${DIFFICULTY_LOSS_MODE}, weight=${DIFFICULTY_WEIGHT}, allocation_scale=${DIFFICULTY_ALLOCATION_SCALE}, mode_margin=${DIFFICULTY_MODE_MARGIN_WEIGHT}, margin_floor=${DIFFICULTY_MARGIN_GATE_FLOOR}, quantile=${DIFFICULTY_QUANTILE_HINGE_WEIGHT}"
echo "cost: mode=${COST_GATE_MODE}, start=${COST_START_STEP}, warmup=${COST_WARMUP_STEPS}, success_gate=${COST_SUCCESS_THRESH_1}->${COST_SUCCESS_THRESH_2}, open=${COST_OPEN_RATE}, close=${COST_CLOSE_RATE}, band=[${NFE_TARGET_LOWER},${NFE_TARGET_UPPER}], target=${TARGET_NFE}, lambda=${ACTOR_COMPUTE_LAMBDA_WARMUP}->${ACTOR_COMPUTE_LAMBDA}, failed_weight=${FAILED_EPISODE_COST_WEIGHT}"
...
python train_dsrl.py --config-name "dsrl_${TASK}.yaml" \
  ...
  ++train.cost_success_thresh_1="$COST_SUCCESS_THRESH_1" \
  ++train.cost_success_thresh_2="$COST_SUCCESS_THRESH_2" \
  ++train.cost_open_rate="$COST_OPEN_RATE" \
  ++train.cost_close_rate="$COST_CLOSE_RATE" \
  ...
  ++train.failed_episode_cost_weight="$FAILED_EPISODE_COST_WEIGHT" \
  ...
  ++train.difficulty_margin_gate_floor="$DIFFICULTY_MARGIN_GATE_FLOOR" \
