...
class ActionChunkWrapper(gymnasium.Env):
	def __init__(self, env, cfg, max_episode_steps=300):
		...
		self.count = 0
		self.pending_chunk_exec = None
		self.reward_discount = float(cfg.train.get("discount", 1.0))
	...
	def step(self, action):
		...
		chunk_requested = action.shape[0]
		if self.pending_chunk_exec is not None:
			chunk_requested = int(np.rint(self.pending_chunk_exec))
			self.pending_chunk_exec = None
		chunk_requested = int(np.clip(chunk_requested, 1, action.shape[0]))
		remaining_steps = max(1, self.max_episode_steps - self.count)
		chunk_exec = int(min(chunk_requested, remaining_steps))
		obs_ = []
		reward_ = []
		reward_discounted_ = []
		done_ = []
		info_ = []
		done_i = False
		for i in range(chunk_exec):
			self.count += 1
			obs_i, reward_i, done_i, info_i = self.env.step(action[i])
			obs_.append(obs_i)
			reward_.append(reward_i)
			reward_discounted_.append((self.reward_discount ** i) * reward_i)
			done_.append(done_i)
			info_.append(info_i)
			if done_i:
				break
		obs = obs_[-1]
		reward = sum(reward_discounted_)
		done = np.max(done_)
		info = info_[-1]
		info['chunk_requested'] = float(chunk_requested)
		info['chunk_exec'] = float(len(reward_))
		info['chunk_exec_over_requested_ratio'] = float(len(reward_) / max(chunk_requested, 1))
		info['chunk_reward_undiscounted'] = float(sum(reward_))
		info['chunk_success_signal'] = float(max(reward_)) if reward_ else float(reward)
